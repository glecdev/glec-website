/**
 * Admin Press List Page
 *
 * Based on: FR-ADMIN-003 (공지사항 목록 조회 - PRESS 카테고리 전용)
 * API: GET /api/admin/notices?category=PRESS
 * Standards: GLEC-Design-System-Standards.md (Table, Pagination, Filter)
 *
 * Features:
 * - Paginated press release list (20 items/page)
 * - Status filter (DRAFT, PUBLISHED, ARCHIVED)
 * - Category: PRESS only (fixed filter)
 * - Search by title
 * - Create press release button
 * - Edit/Delete actions
 */
...
export default function AdminPressPage() {
   ...
   * IMPORTANT: category=PRESS is always set (fixed filter)
   */
  useEffect(() => {
    fetchPressReleases();
  }, [page, status, search]);
  ...
  const fetchPressReleases = async () => {
    try {
      ...
      // Build query params - PRESS category is ALWAYS applied
      const params = new URLSearchParams({
        page: page.toString(),
        per_page: '20',
        category: 'PRESS', // Fixed filter
      });
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : 'Failed to delete press release');
    }
  };
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          ...
          <Link
            href="/admin/notices/new?category=PRESS"
                    ...
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-gray-500 font-medium">보도자료가 없습니다.</p>
                          <Link
                            href="/admin/notices/new?category=PRESS"
                    ...
                    pressReleases.map((press) => (
                      <tr key={press.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4">
                          <Link
                            href={`/admin/notices/edit?id=${press.id}`}
            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                {Array.from({ length: Math.min(meta.total_pages, 10) }, (_, i) => {
                  ...
                  if (meta.total_pages > 10) {
                    if (i < 5 || i >= meta.total_pages - 5) {
                      const pageNum = i < 5 ? i + 1 : meta.total_pages - (9 - i);
                      return (
                        <button
                          key={pageNum}

...
export const GET = withAuth(
  async (req: NextRequest) => {
    try {
      ...
      const timeSeries = await generateTimeSeries(sql, dateFrom, dateTo, granularity);

      // 2. SCORE DISTRIBUTION: Histogram of lead scores (0-100 in buckets of 10)
      const scoreDistribution = await generateScoreDistribution(sql, dateFrom, dateTo);

      // 3. STATUS DISTRIBUTION: Count by status
      const statusDistribution = await generateStatusDistribution(sql, dateFrom, dateTo);

      // 4. SOURCE DISTRIBUTION: Count by lead source type
      const sourceDistribution = await generateSourceDistribution(sql, dateFrom, dateTo);

      // 5. CONVERSION FUNNEL: Stages from created → qualified → converted
      const conversionFunnel = await generateConversionFunnel(sql, dateFrom, dateTo);

      // 6. EMAIL ENGAGEMENT: Open/click rates over time
      const emailEngagement = await generateEmailEngagement(sql, dateFrom, dateTo, granularity);

      // 7. TOP PERFORMERS: Best performing templates and rules
      const topPerformers = await generateTopPerformers(sql, dateFrom, dateTo);

...
async function createTestEvent() {
  ...
  const uniqueSlug = `e2e-test-event-${Date.now()}`;
  ...
  if (!data.success) {
    ...
    throw new Error(`Event creation failed: ${data.error?.message || JSON.stringify(data.error)}`);
  }
...
async function testStatusUpdate(registrationId, newStatus) {
  console.log(`Step 5-${newStatus}: Update Status to ${newStatus}...`);
...
async function cleanup() {
  ...
  const response = await fetch(
    `${BASE_URL}/api/admin/events?id=${createdEventId}`,
    {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${adminToken}`

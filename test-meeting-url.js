...
async function test() {
  ...
  if (proposalResponse.status === 200 && proposalResponse.data.success) {
    ...
    console.log('📧 응답 데이터:', JSON.stringify(proposalResponse.data, null, 2));
    if (proposalResponse.data.data && proposalResponse.data.data.booking_url) {
      console.log(`\n🔗 생성된 URL: ${proposalResponse.data.data.booking_url}`);
      const url = new URL(proposalResponse.data.data.booking_url);
      console.log(`🔍 URL 호스트: ${url.host}`);
      console.log(`🔍 URL 포트: ${url.port || '80'}`);
    }
  } else {
